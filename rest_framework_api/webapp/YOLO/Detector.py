...
class Detector:
	...
	def draw_pred(self,img, class_id, confidence, x, y, x_plus_w, y_plus_h):
	...
	#image=cv2.resize(image, (1920, 416))
	    image = self.adjust_gamma(image, gamma=1)
	    ...
	    for i in indices:
	        ...
	        w = box[2]
	        h = box[3]
	        self.draw_pred(image, class_ids[i], confidences[i], round(x), round(y), round(x+w), round(y+h))
